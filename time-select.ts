...
export class TimeSelectComponent implements OnInit {
  ...
  setFromTimeOptions(): void {
    ...
    let time: TimeSpan = TimeSpan.getOnString(this.params.endTime.toString()); 
    //new TimeSpan(this.params.endTime.hours, this.params.endTime.minutes);
...
/*
From
<select [(ngModel)]="params.startTime"
        (change)="setToTimeOptions()">
  <option *ngFor="let op of fromOptions"
          [value]="op.value"
...
To
<select [(ngModel)]="params.endTime"
        (change)="setFromTimeOptions()">
  <option *ngFor="let op of toOptions"
          [value]="op.value"
          [disabled]="op.isDisabled">
    {{op.label}}
  </option>
